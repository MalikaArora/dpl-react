...
const ShowNonLinearWizard = () => {
  ...
  else
  {
    return (  
        ...
        <div>
          <Wizard cancelDeletion={() => clearState()} onSaveClicked={() => saveFunction()} onSubmitClicked={((selected2&&(selected&&(treetyp != 0))) ? "true" : "false")} submitFunctionDefined={() => submitFunction()} nonlinear="true">
            <Wizard.Step name="1.Introduction" key="1">
              <p>The Non-Linear Wizard can be navigated through using the next/previous buttons and the step headers. Steps can be completed in any sequence.</p>
            </Wizard.Step>
...
export default ShowNonLinearWizard;
